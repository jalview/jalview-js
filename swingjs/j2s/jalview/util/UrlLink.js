(function(){var P$=Clazz.newPackage("jalview.util"),I$=[[0,'com.stevesoft.pat.Regex','java.util.Vector','java.util.Arrays','jalview.util.DBRefUtils','java.util.Collections']],I$0=I$[0],$I$=function(i,n,m){return m?$I$(i)[n].apply(null,m):((i=(I$[i]||(I$[i]=Clazz.load(I$0[i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
linkset.put$O$O(this.label + "|" + this.getUrlPrefix$() , $I$(3,"asList$OA",[Clazz.array(String, -1, [this.target, this.label, null, this.getUrlPrefix$()])]));
}});
...
this.createBareURLLink$S$Boolean$java_util_Map(dbr.get$I(r).getAccessionId$(), Boolean.valueOf$Z(true), linkset);
}
}} else if (!this.usesDBAccession$() && id != null  ) {
this.createBareURLLink$S$Boolean$java_util_Map(id, Boolean.valueOf$Z(false), linkset);
}if (descr != null  && this.getRegexReplace$() != null  ) {
this.createBareURLLink$S$Boolean$java_util_Map(descr, Boolean.valueOf$Z(false), linkset);
}});
...
}linkset.put$O$O(urls[u] + "|" + urls[u + 1] , $I$(3,"asList$OA",[Clazz.array(String, -1, [this.target, thisLabel, urls[u], urls[u + 1]])]));
}}
}});
