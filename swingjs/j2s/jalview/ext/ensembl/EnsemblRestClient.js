(function(){var P$=Clazz.newPackage("jalview.ext.ensembl"),p$1={},I$=[[0,'jalview.util.Platform','java.util.HashMap','jalview.ext.ensembl.EnsemblData','java.net.URL','Thread','StringBuilder','java.io.DataOutputStream','jalview.util.StringUtils']],I$0=I$[0],$I$=function(i,n){return((i=(I$[i]||(I$[i]=Clazz.load(I$0[i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
C$.domainData.put$O$O(this.ensemblDomain, Clazz.new_($I$(3,1).c$$S$S,[this.ensemblDomain, "15.2"]));
}if (!C$.domainData.containsKey$O(this.ensemblGenomesDomain)) {
C$.domainData.put$O$O(this.ensemblGenomesDomain, Clazz.new_($I$(3,1).c$$S$S,[this.ensemblGenomesDomain, "15.2"]));
}}, 1);
...
if (retryDelay != null ) {
try {
var retrySecs=(Integer.valueOf$S(retryDelay)).$c();
...
var retestAvailability=Long.$gt((Long.$sub(now,info.lastAvailableCheckTime)),10000 );
if (!info.restAvailable || retestAvailability ) {
info.restAvailable=this.checkEnsembl$();
info.lastAvailableCheckTime=now;
}var refetchVersion=Long.$gt((Long.$sub(now,info.lastVersionCheckTime)),3600000 );
if (refetchVersion) {
...
try {
if ((Float.valueOf$S(majorVersion)).$c() > (Float.valueOf$S(expectedMajorVersion)).$c() ) {
...
if (laterVersion) {
System.err.println$S(String.format$S$OA("EnsemblRestClient expected %s REST version %s but found %s, see %s", Clazz.array(java.lang.Object, -1, [this.getDbSource$(), expected, version, "https://github.com/Ensembl/ensembl-rest/wiki/Change-log"])));
...
C$.domainData.put$O$O("https://rest.ensembl.org", Clazz.new_($I$(3,1).c$$S$S,["https://rest.ensembl.org", "15.2"]));
C$.domainData.put$O$O("https://rest.ensembl.org", Clazz.new_($I$(3,1).c$$S$S,["https://rest.ensembl.org", "15.2"]));
};
};
})();
