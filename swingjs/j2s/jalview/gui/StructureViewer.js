...
if (viewer != null ) {
return viewer;
}var viewerType=C$.getViewerType$();
...
), Clazz.new_(P$.StructureViewer$1.$init$,[this, {ap:ap,theSeqs:theSeqs,pdbsForFile:pdbsForFile}]))],$I$(5,1).c$$Runnable).start$();
return this.sview;
...
} else if (viewerType.equals$O($I$(2).CHIMERAX)) {
this.sview=Clazz.new_($I$(8,1).c$$jalview_datamodel_PDBEntryA$Z$jalview_datamodel_SequenceIAA$jalview_gui_AlignmentPanel,[pdbsForFile, this.superposeAdded, theSeqs, ap]);
} else if (viewerType.equals$O($I$(2).PYMOL)) {
this.sview=Clazz.new_($I$(9,1).c$$jalview_datamodel_PDBEntryA$Z$jalview_datamodel_SequenceIAA$jalview_gui_AlignmentPanel,[pdbsForFile, this.superposeAdded, theSeqs, ap]);
} else {
$I$(10,"error$S",["Unknown structure viewer type " + C$.getViewerType$().toString()]);
}return this.sview;
});
...
return null;
}var pdbsSeen=Clazz.new_($I$(11,1));
var pdbSeqs=Clazz.new_($I$(12,1));
...
pdbSeqs.put$O$O(pdb, Clazz.new_($I$(13,1)));
} else {
...
this.sview=Clazz.new_($I$(8,1).c$$jalview_datamodel_PDBEntry$jalview_datamodel_SequenceIA$SA$jalview_gui_AlignmentPanel,[pdb, seqsForPdb, null, ap]);
} else if (viewerType.equals$O($I$(2).PYMOL)) {
this.sview=Clazz.new_($I$(9,1).c$$jalview_datamodel_PDBEntry$jalview_datamodel_SequenceIA$O$jalview_gui_AlignmentPanel,[pdb, seqsForPdb, null, ap]);
} else {
$I$(10,"error$S",["Unknown structure viewer type " + C$.getViewerType$().toString()]);
}return this.sview;
});

Clazz.newMeth(C$, 'createView$jalview_gui_StructureViewer_ViewerType$jalview_gui_AlignmentPanel$jalview_datamodel_StructureViewerModel$S$S',  function (type, alignPanel, viewerData, sessionFile, vid) {
var viewer=null;
switch (type) {
case $I$(2).JMOL:
viewer=Clazz.new_($I$(6,1).c$$jalview_datamodel_StructureViewerModel$jalview_gui_AlignmentPanel$S$S,[viewerData, alignPanel, sessionFile, vid]);
break;
case $I$(2).CHIMERA:
viewer=Clazz.new_($I$(7,1).c$$jalview_datamodel_StructureViewerModel$jalview_gui_AlignmentPanel$S$S,[viewerData, alignPanel, sessionFile, vid]);
break;
case $I$(2).CHIMERAX:
viewer=Clazz.new_($I$(8,1).c$$jalview_datamodel_StructureViewerModel$jalview_gui_AlignmentPanel$S$S,[viewerData, alignPanel, sessionFile, vid]);
break;
case $I$(2).PYMOL:
viewer=Clazz.new_($I$(9,1).c$$jalview_datamodel_StructureViewerModel$jalview_gui_AlignmentPanel$S$S,[viewerData, alignPanel, sessionFile, vid]);
break;
default:
$I$(10,"error$S",["Unknown structure viewer type " + type.toString()]);
}
return viewer;
}, 1);
...
Clazz.newEnumConst($vals, C$.c$, "CHIMERAX", 2, []);
Clazz.newEnumConst($vals, C$.c$, "PYMOL", 3, []);
};
