...
rj.error=true;
$I$(10,"fatal$S",["Unexpected REST Job " + p$1.getStage$jalview_ws_rest_RestJobThread_Stage.apply(this, [stg]) + "exception for URL " + rj.rsd.postUrl ]);
throw (he);
}
...
$I$(10,"warn$S",["IO Exception for REST Job " + p$1.getStage$jalview_ws_rest_RestJobThread_Stage.apply(this, [stg]) + "exception for URL " + rj.rsd.postUrl ]);
throw (e);
}
} else {
throw e$$;
...
case 200:
rj.running=false;
$I$(10).debug$S("Processing result set.");
...
if (loc.length > 1) {
$I$(10,"warn$S",["Ignoring additional " + (loc.length - 1) + " location(s) provided in response header ( next one is '" + loc[1].getValue$() + "' )" ]);
...
break;
default:
$I$(10,"warn$S",["Unhandled response status when " + p$1.getStage$jalview_ws_rest_RestJobThread_Stage.apply(this, [stg]) + "for " + postUrl + ": " + response.getStatusLine$() ]);
rj.error=true;
rj.setAllowedServerExceptions$I(0);
rj.setSubjobComplete$Z(true);
rj.setSubmitted$Z(true);
...
$I$(10).debug$S$Throwable("IOException when consuming unhandled response", e);
} else {
throw e;
}
}
...
f=f.toLowerCase$java_util_Locale($I$(13).ROOT);
...
$I$(10).error$S$Throwable("Exception when trying to start Rest Job.", ex);
}
} else {
throw e$$;
}
...
$I$(10).warn$S("Rest job result parser is currently INCOMPLETE!");
var validres=0;
for (var rj, $rj = 0, $$rj = this.jobs; $rj<$$rj.length&&((rj=($$rj[$rj])),1);$rj++) {
if (rj.hasResponse$() && rj.resSet != null   && rj.resSet.isValid$() ) {
var ln=null;
try {
$I$(10,"debug$S",["Parsing data for job " + rj.getJobId$()]);
rj.parseResultSet$();
if (rj.hasResults$()) {
++validres;
}$I$(10,"debug$S",["Finished parsing data for job " + rj.getJobId$()]);
} catch (e$$) {
if (Clazz.exceptionOf(e$$,"Error")){
var ex = e$$;
{
$I$(10,"warn$S",["Failed to finish parsing data for job " + rj.getJobId$()]);
ex.printStackTrace$();
}
} else if (Clazz.exceptionOf(e$$,"Exception")){
var ex = e$$;
...
this.wsInfo.setStatus$I($I$(14).STATE_STOPPED_ERROR);
this.wsInfo.setFinishedNoResults$();
}});

Clazz.newMeth(C$, 'realiseResults$Z',  function (merge) {
var destPanels=Clazz.new_($I$(15,1));
var resultDest=Clazz.new_($I$(15,1));
...
var numAlSets=0;
var als=0;
var destAls=Clazz.new_($I$(15,1));
var destColsel=Clazz.new_($I$(15,1));
var trees=Clazz.new_($I$(15,1));
do {
var ordermap=Clazz.array(Integer.TYPE, [this.jobs.length, null]);
var rseqs=Clazz.array($I$(16), [this.jobs.length, null]);
var orders=Clazz.array($I$(17), [this.jobs.length]);
var alan=Clazz.array($I$(18), [this.jobs.length, null]);
var sgrp=Clazz.array($I$(19), [this.jobs.length, null]);
for (var j=0; j < this.jobs.length; j++) {
var rj=this.jobs[j];
if (rj.hasResults$()) {
var rset=rj.context;
...
sgrp[j]=Clazz.array($I$(19), [alset.al.getGroups$().size$()]);
alset.al.getGroups$().toArray$OA(sgrp[j]);
} else {
sgrp[j]=null;
}orders[j]=Clazz.new_($I$(17,1).c$$jalview_datamodel_AlignmentI,[alset.al]);
rseqs[j]=alset.al.getSequencesArray$();
ordermap[j]=rj.getOrderMap$();
if (alset.trees != null ) {
trees.add$O(Clazz.new_($I$(15,1).c$$java_util_Collection,[alset.trees]));
} else {
trees.add$O(Clazz.new_($I$(15,1)));
}}}}}
var nvertsep=0;
var nvertseps=1;
if (vsepjobs) {
}var vrestjob=0;
var visgrps=Clazz.new_($I$(15,1));
var groupNames=Clazz.new_($I$(20,1));
var visAlAn=null;
...
resultDest.add$O(this.restClient.isShowResultsInNewView$() ? $I$(21).newView : $I$(21).currentView);
destPanels.add$O(this.restClient.recoverAlignPanelForView$());
} else {
newAlignment=true;
var idat=this.input.getAlignmentAndHiddenColumns$C(gapCharacter);
destAl=Clazz.new_($I$(22,1).c$$jalview_datamodel_SequenceIA,[idat[0]]);
destHCs=idat[1];
resultDest.add$O($I$(21).newAlignment);
}destAls.add$O(destAl);
destColsel.add$O(destHCs);
}} else {
if (destAls.size$() > als) {
if (!vsepjobs) {
$I$(10).warn$S("dealing with multiple alignment products returned by non-vertically separable service.");
}destAl=destAls.get$I(als);
...
var alo=Clazz.array($I$(17), [nsq.length]);
for (var no=0; no < alo.length; no++) {
alo[no]=Clazz.new_([orders[nrj].getOrder$()],$I$(17,1).c$$java_util_List);
}
...
}destAl=Clazz.new_($I$(22,1).c$$jalview_datamodel_SequenceIA,[newview[0]]);
destHCs=newview[1];
newAlignment=true;
destAls.add$O(destAl);
destColsel.add$O(destHCs);
resultDest.add$O($I$(21).newAlignment);
throw Clazz.new_([$I$(24).getString$S("error.implementation_error") + "TODO: "],$I$(23,1).c$$S);
}}var initnrj=nrj;
...
exsg=Clazz.new_($I$(19,1).c$$jalview_datamodel_SequenceGroup,[sg]);
...
$I$(10).warn$S("Couldn\'t resolve original sequence for new sequence.");
}}
if (sg.hasSeqrep$()) {
...
if (grass == null ) {
$I$(10,"error$S",["Couldn't relocate group referemce for group " + alan[nrj][an].groupRef.getName$()]);
}}if (visAlAn == null ) {
visAlAn=Clazz.new_($I$(15,1));
}var visan=null;
...
if (visan == null ) {
visan=Clazz.new_($I$(18,1).c$$jalview_datamodel_AlignmentAnnotation,[alan[nrj][an]]);
visan.annotations=Clazz.array($I$(25), [this.input.getWidth$()]);
visan.groupRef=grass;
visan.sequenceRef=sqass;
visAlAn.add$O(visan);
}if (contigs[ncnt] + alan[nrj][an].annotations.length > visan.annotations.length) {
var newannv=Clazz.array($I$(25), [contigs[ncnt] + alan[nrj][an].annotations.length]);
...
$I$(10).error$S("Tree recovery from restjob not yet implemented.");
}
}}
}}
if (visAlAn != null ) {
...
var destal;
var destcs;
var alTitle=$I$(24,"formatMessage$S$SA",["label.webservice_job_title_on", Clazz.array(String, -1, [this.restClient.service.details.Action, this.restClient.service.details.Name, this.restClient.viewTitle])]);
switch (action) {
case $I$(21).newAlignment:
destal=destAls.get$I(als);
destcs=destColsel.get$I(als);
destaf=Clazz.new_($I$(26,1).c$$jalview_datamodel_AlignmentI$jalview_datamodel_HiddenColumns$I$I,[destal, destcs, 700, 500]);
$I$(27,"Refresh$java_awt_Component$S",[destaf, destaf.getViewport$().getSequenceSetId$()]);
$I$(28).addInternalFrame$javax_swing_JInternalFrame$S$I$I(destaf, alTitle, 700, 500);
break;
case $I$(21).newView:
break;
case $I$(21).currentView:
break;
}
}
if (!newAlignment) {
...
var blocks=Clazz.array($I$(16), [nvc, null]);
if (contigs == null ) {
blocks[0]=Clazz.array($I$(16), [sequenceIs.length]);
...
var _warnings=Clazz.new_($I$(15,1));
var validt=true;
if (this.jobs != null ) {
for (var rj, $rj = 0, $$rj = this.jobs; $rj<$$rj.length&&((rj=($$rj[$rj])),1);$rj++) {
if (!rj.hasValidInput$()) {
