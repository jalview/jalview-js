(function(){var P$=Clazz.newPackage("jalview.ws.jws1"),p$1={},I$=[[0,'java.beans.PropertyChangeSupport','ext.vamsas.IRegistryServiceLocator','jalview.bin.Console','java.util.Vector','jalview.bin.Cache','java.util.StringTokenizer','java.net.URL','java.util.Hashtable','ext.vamsas.ServiceHandle','jalview.util.MessageManager','jalview.gui.Desktop','jalview.gui.JvOptionPane','jalview.ws.jws1.Discoverer','Thread','jalview.ws.jws1.MsaWSClient','jalview.ws.jws1.JPredClient','jalview.ws.jws1.SeqSearchWSClient']],I$0=I$[0],$I$=function(i,n,m){return m?$I$(i)[n].apply(null,m):((i=(I$[i]||(I$[i]=Clazz.load(I$0[i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
$I$(3).error$S$Throwable("Serious!  Service location failed\nfor URL :" + WsURL + "\n" , ex);
return null;
} else {
throw ex;
}
...
var RootServiceURLs=$I$(5).getDefault$S$S("DISCOVERY_URLS", "http://www.compbio.dundee.ac.uk/JalviewWS/services/ServiceRegistry");
try {
var st=Clazz.new_($I$(6,1).c$$S$S,[RootServiceURLs, ","]);
while (st.hasMoreElements$()){
var url=null;
try {
var u=Clazz.new_([url=st.nextToken$()],$I$(7,1).c$$S);
if (!urls.contains$O(u)) {
urls.add$O(u);
} else {
$I$(3).info$S("Ignoring duplicate url in DISCOVERY_URLS list");
}} catch (ex) {
if (Clazz.exceptionOf(ex,"Exception")){
$I$(3,"warn$S",["Problem whilst trying to make a URL from '" + ((url != null ) ? url : "<null>") + "'" ]);
$I$(3,"warn$S",["This was probably due to a malformed comma separated list in the DISCOVERY_URLS entry of $(HOME)/.jalview_properties)"]);
...
if (Clazz.exceptionOf(ex,"Exception")){
$I$(3).warn$S$Throwable("Error parsing comma separated list of urls in DISCOVERY_URLS.", ex);
} else {
throw ex;
}
}
...
$I$(3,"debug$S",["(Re)-Initialising the discovery URL list."]);
try {
C$.reallyDiscoverServices=$I$(5).getDefault$S$Z("DISCOVERY_START", false);
if (C$.reallyDiscoverServices) {
C$.ServiceURLList=C$.getDiscoveryURLS$();
} else {
$I$(3).debug$S("Setting default services");
C$.services=Clazz.new_($I$(8,1));
var defServices=Clazz.array($I$(9), -1, [Clazz.new_(["MsaWS", "Edgar, Robert C. (2004), MUSCLE: multiple sequence alignment with high accuracy and high throughput, Nucleic Acids Research 32(5), 1792-97.", "http://www.compbio.dundee.ac.uk/JalviewWS/services/MuscleWS", $I$(10).getString$S("label.muscle_multiple_protein_sequence_alignment")],$I$(9,1).c$$S$S$S$S), Clazz.new_(["MsaWS", "Katoh, K., K. Kuma, K., Toh, H.,  and Miyata, T. (2005) \"MAFFT version 5: improvement in accuracy of multiple sequence alignment.\" Nucleic Acids Research, 33 511-518", "http://www.compbio.dundee.ac.uk/JalviewWS/services/MafftWS", $I$(10).getString$S("label.mafft_multiple_sequence_alignment")],$I$(9,1).c$$S$S$S$S), Clazz.new_(["MsaWS", "Thompson, J.D., Higgins, D.G. and Gibson, T.J. (1994) CLUSTAL W: improving the sensitivity of progressive multiple sequence alignment through sequence weighting, position specific gap penalties and weight matrix choice. Nucleic Acids Research, 22 4673-4680", "http://www.compbio.dundee.ac.uk/JalviewWS/services/ClustalWS", $I$(10).getString$S("label.clustalw_multiple_sequence_alignment")],$I$(9,1).c$$S$S$S$S), Clazz.new_(["SecStrPred", "Drozdetskiy A, Cole C, Procter J & Barton GJ. (2015)\nJPred4: a protein secondary structure prediction server\nNucleic Acids Research, Web Server issue (first published 15th April 2015)\ndoi://10.1093/nar/gkv332", "http://www.compbio.dundee.ac.uk/JalviewWS/services/jpred", "JPred Secondary Structure Prediction"],$I$(9,1).c$$S$S$S$S)]);
C$.services=Clazz.new_($I$(8,1));
...
var shs=null;
try {
$I$(3).debug$S("Discovering services using " + location);
...
if ($I$(11).desktop != null ) {
$I$(12,"showMessageDialog$java_awt_Component$S$S$I",[$I$(11).desktop, $I$(10).getString$S("label.set_proxy_settings"), $I$(10).getString$S("label.proxy_authorization_failed"), 2]);
}} else {
$I$(3).warn$S("No Discovery service at " + location);
$I$(3).debug$S$Throwable("Axis Fault", f);
}}
} else if (Clazz.exceptionOf(e$$,"Exception")){
var e = e$$;
{
$I$(3).warn$S("No Discovery service at " + location);
$I$(3).debug$S$Throwable("Discovery Service General Exception", e);
}
} else {
throw e$$;
}
...
$I$(3,"debug$S",["A " + sh[i].getAbstractName$() + " service called " + sh[i].getName$() + " exists at " + sh[i].getEndpointURL$() + "\n" ]);
...
var disc_serv=null;
try {
disc_serv=Clazz.new_([sh[i].getEndpointURL$()],$I$(7,1).c$$S);
if (!C$.ServiceURLList.contains$O(disc_serv)) {
$I$(3).debug$S("Adding new discovery service at " + disc_serv);
C$.ServiceURLList.add$O(disc_serv);
seenNewDiscovery=true;
}} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
$I$(3,"debug$S$Throwable",["Ignoring bad discovery service URL " + sh[i].getEndpointURL$(), e]);
} else {
throw e;
}
}
...
var sscat=Clazz.new_($I$(8,1));
var cat=Clazz.new_($I$(4,1));
var sh=null;
var s_url=0;
if (C$.ServiceURLList == null ) {
$I$(3).debug$S("No service endpoints to use for service discovery.");
return;
...
$I$(3,"warn$S",["No services at " + (C$.ServiceURLList.get$I(s_url)) + " - check DISCOVERY_URLS property in .jalview_properties" ]);
}++s_url;
}
...
), Clazz.new_($I$(14,1),[this, {discoverer:discoverer}],P$.Discoverer$1));
discoverThread.start$();
});

Clazz.newMeth(C$, 'getServiceClient$ext_vamsas_ServiceHandle',  function (sh) {
if (C$.serviceClientBindings == null ) {
C$.serviceClientBindings=Clazz.new_($I$(8,1));
C$.serviceClientBindings.put$O$O("MsaWS", Clazz.new_($I$(15,1)));
C$.serviceClientBindings.put$O$O("SecStrPred", Clazz.new_($I$(16,1)));
C$.serviceClientBindings.put$O$O("SeqSearch", Clazz.new_($I$(17,1)));
