(function(){var P$=Clazz.newPackage("jalview.ws.dbsources"),I$=[[0,'com.stevesoft.pat.Regex','jalview.ws.ebi.EBIFetchClient','java.util.Locale','java.io.FileInputStream','java.util.ArrayList','jalview.datamodel.Alignment','jalview.datamodel.SequenceI','javax.xml.bind.JAXBContext','javax.xml.stream.XMLInputFactory','jalview.xml.binding.embl.ROOT','jalview.datamodel.Sequence','jalview.datamodel.DBRefEntry','jalview.datamodel.Mapping','jalview.util.DBRefUtils','jalview.analysis.SequenceIdMatcher','jalview.datamodel.FeatureProperties','java.util.Hashtable','jalview.util.MappingUtils','jalview.util.MapList','jalview.datamodel.DBRefSource','jalview.util.DnaUtils','jalview.bin.Console','jalview.datamodel.SequenceFeature','java.util.Arrays']],I$0=I$[0],$I$=function(i,n,m){return m?$I$(i)[n].apply(null,m):((i=(I$[i]||(I$[i]=Clazz.load(I$0[i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
var reply;
try {
reply=dbFetch.fetchDataAsFile$S$S$S(emprefx.toLowerCase$java_util_Locale($I$(3).ROOT) + ":" + query.trim$() , "display=xml", "xml");
} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
this.stopQuery$();
throw Clazz.new_(Clazz.load('Exception').c$$S$Throwable,[String.format$S$OA("EBI EMBL XML retrieval failed for %s:%s", Clazz.array(java.lang.Object, -1, [emprefx.toLowerCase$java_util_Locale($I$(3).ROOT), query.trim$()])), e]);
} else {
throw e;
}
}
...
if (Long.$gt(reply.length$(),"ERROR 12 No entries found.".length$() )) {
var is=Clazz.new_($I$(4,1).c$$java_io_File,[reply]);
entries=this.getEmblEntries$java_io_InputStream(is);
}}var al=null;
var seqs=Clazz.new_($I$(5,1));
var peptides=Clazz.new_($I$(5,1));
if (entries != null ) {
...
var dna=Clazz.new_($I$(11,1).c$$S$S,[sourceDb + "|" + accession , seqString]);
dna.setDescription$S(entry.getDescription$());
var sequenceVersion=String.valueOf$I(entry.getVersion$().intValue$());
var selfRref=Clazz.new_($I$(12,1).c$$S$S$S,[sourceDb, sequenceVersion, accession]);
dna.addDBRef$jalview_datamodel_DBRefEntry(selfRref);
selfRref.setMap$jalview_datamodel_Mapping(Clazz.new_([null, Clazz.array(Integer.TYPE, -1, [1, dna.getLength$()]), Clazz.array(Integer.TYPE, -1, [1, dna.getLength$()]), 1, 1],$I$(13,1).c$$jalview_datamodel_SequenceI$IA$IA$I$I));
...
version="0";
}dna.addDBRef$jalview_datamodel_DBRefEntry(Clazz.new_($I$(12,1).c$$S$S$S,[source, version, acc]));
}
}var matcher=Clazz.new_($I$(15,1).c$$java_util_List,[peptides]);
try {
...
if (product == null ) {
product=Clazz.new_($I$(11,1).c$$S$S$I$I,[proteinId, translation, 1, translationLength]);
...
dnaToProteinMapping=Clazz.new_([product, exons, Clazz.array(Integer.TYPE, -1, [1, translationLength]), 3, 1],$I$(13,1).c$$jalview_datamodel_SequenceI$IA$IA$I$I);
...
var exons2=C$.adjustForProteinLength$I$IA(translationLength, exons);
dnaToProteinMapping=Clazz.new_([product, exons2, Clazz.array(Integer.TYPE, -1, [1, translationLength]), 3, 1],$I$(13,1).c$$jalview_datamodel_SequenceI$IA$IA$I$I);
if (product != null ) {
var proteinToEmblRef=Clazz.new_(["EMBL", sequenceVersion, proteinId, Clazz.new_([dnaToProteinMapping.getMap$().getInverse$()],$I$(13,1).c$$jalview_util_MapList)],$I$(12,1).c$$S$S$S$jalview_datamodel_Mapping);
product.addDBRef$jalview_datamodel_DBRefEntry(proteinToEmblRef);
var proteinToCdsMapList=Clazz.new_([Clazz.array(Integer.TYPE, -1, [1, translationLength]), Clazz.array(Integer.TYPE, -1, [1 + (codonStart - 1), (codonStart - 1) + 3 * translationLength]), 1, 3],$I$(19,1).c$$IA$IA$I$I);
var proteinToEmblCdsRef=Clazz.new_(["EMBLCDS", sequenceVersion, proteinId, Clazz.new_($I$(13,1).c$$jalview_util_MapList,[proteinToCdsMapList])],$I$(12,1).c$$S$S$S$jalview_datamodel_Mapping);
product.addDBRef$jalview_datamodel_DBRefEntry(proteinToEmblCdsRef);
proteinToEmblProteinRef=Clazz.new_($I$(12,1).c$$jalview_api_DBRefEntryI,[proteinToEmblCdsRef]);
...
var desc=String.format$S$OA("Exon %d for protein \'%s\' EMBLCDS:%s", Clazz.array(java.lang.Object, -1, [Integer.valueOf$I(exonNumber), proteinName, proteinId]));
...
if (version == null  || "".equals$O(version) ) {
version="0";
}var dbref=Clazz.new_([source, version, xref.getId$()],$I$(12,1).c$$S$S$S);
var proteinDbRef=Clazz.new_([source, version, dbref.getAccessionId$()],$I$(12,1).c$$S$S$S);
...
if (mappingUsed) {
dnaToProteinMapping=Clazz.new_($I$(13,1).c$$jalview_datamodel_Mapping,[dnaToProteinMapping]);
}mappingUsed=true;
var proteinSeq=matcher.findIdMatch$S(proteinSeqName);
if (proteinSeq == null ) {
proteinSeq=Clazz.new_([proteinSeqName, product.getSequenceAsString$()],$I$(11,1).c$$S$S);
matcher.add$jalview_datamodel_SequenceI(proteinSeq);
proteinSeq.setDescription$S(product.getDescription$());
...
var pmap=Clazz.new_([dna, dnaToProteinMapping.getMap$().getInverse$()],$I$(13,1).c$$jalview_datamodel_SequenceI$jalview_util_MapList);
pref=Clazz.new_($I$(12,1).c$$S$S$S,[sourceDb, sequenceVersion, accession]);
...
proteinToEmblProteinRef=Clazz.new_([$I$(20).EMBLCDSProduct, sequenceVersion, proteinId],$I$(12,1).c$$S$S$S);
}product.addDBRef$jalview_datamodel_DBRefEntry(proteinToEmblProteinRef);
if (dnaToProteinMapping != null  && dnaToProteinMapping.getTo$() != null  ) {
var dnaToEmblProteinRef=Clazz.new_([$I$(20).EMBLCDSProduct, sequenceVersion, proteinId],$I$(12,1).c$$S$S$S);
...
$I$(22,"warn$S",[String.format$S$OA("Not parsing inexact CDS location %s in ENA %s", Clazz.array(java.lang.Object, -1, [location, accession]))]);
return Clazz.array(Integer.TYPE, -1, []);
} else {
throw e;
}
...
var sf=Clazz.new_($I$(23,1).c$$S$S$I$I$S,[type, desc, begin, end, group]);
if (!vals.isEmpty$()) {
...
var exonLength=$I$(18,"getLength$java_util_List",[$I$(24,"asList$OA",[Clazz.array(Integer.TYPE, -1, [exon])])]);
