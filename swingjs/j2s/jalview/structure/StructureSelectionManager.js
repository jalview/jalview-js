...
var progressSessionId=Long.$mul(System.currentTimeMillis$(),3);
...
for (var s, $s = 0, $$s = sequenceArray; $s<$$s.length&&((s=($$s[$s])),1);$s++) {
if (s.isProtein$()) {
isProtein=true;
break;
}}
isMapUsingSIFTs=isMapUsingSIFTs && pdb.isPPDBIdAvailable$() && !pdb.getId$().startsWith$S("AF-") && isProtein  ;
} catch (ex) {
...
$I$(12).error$S$Throwable("SIFTS mapping failed", e);
$I$(12).error$S("Falling back on Needleman & Wunsch alignment");
siftsClient=null;
} else {
throw e;
}
...
var as=$I$(13).doGlobalNWAlignment$jalview_datamodel_SequenceI$jalview_datamodel_SequenceI$S(seq, chain.sequence, type);
...
maxChain.transferRESNUMFeatures$jalview_datamodel_SequenceI$S$S(seq, "IEA: SIFTS", pdb.getId$().toLowerCase$java_util_Locale($I$(14).ROOT));
...
maxChain.transferRESNUMFeatures$jalview_datamodel_SequenceI$S$S(seq, "IEA:Jalview", pdb.getId$().toLowerCase$java_util_Locale($I$(14).ROOT));
...
chain.transferRESNUMFeatures$jalview_datamodel_SequenceI$S$S(seq, "IEA: SIFTS", pdb.getId$().toLowerCase$java_util_Locale($I$(14).ROOT));
...
seqToStrucMapping.add$O(nwMapping);
maxChain.transferRESNUMFeatures$jalview_datamodel_SequenceI$S$S(seq, null, pdb.getId$().toLowerCase$java_util_Locale($I$(14).ROOT));
maxChain.transferResidueAnnotation$jalview_structure_StructureMapping$jalview_datamodel_Mapping(nwMapping, sqmpping);
ds.addPDBId$jalview_datamodel_PDBEntry(maxChain.sequence.getAllPDBEntries$().get$I(0));
}}} else {
if (progress != null ) {
...
if ($I$(15,"isCalcIdForFile$jalview_datamodel_AlignmentAnnotation$S",[ala, this.findIdForPDBFile$S(pdbFile)])) {
parseSecStr=false;
}}
}}
}return parseSecStr;
...
var mappingDetails=Clazz.new_($I$(16,1).c$$I,[128]);
...
this.$finals$.mappingDetails.append$S($I$(17).NEWLINE);
});
})()
), Clazz.new_($I$(18,1).c$$java_io_OutputStream,[this, {mappingDetails:mappingDetails}, System.out],P$.StructureSelectionManager$1));
...
mapping.put$O$O(Integer.valueOf$I(tmp.alignmentMapping + 1), Clazz.array(Integer.TYPE, -1, [tmp.resNumber, tmp.atomIndex]));
}}++index;
} while (index < maxChain.atoms.size$());
var nwMapping=Clazz.new_([seq, pdbFile, pdb.getId$(), maxChainId, mapping, mappingDetails.toString()],$I$(19,1).c$$jalview_datamodel_SequenceI$S$S$S$java_util_HashMap$S);
maxChain.transferResidueAnnotation$jalview_structure_StructureMapping$jalview_datamodel_Mapping(nwMapping, sqmpping);
return nwMapping;
}, p$1);
...
return;
}var pdbs=Clazz.new_([$I$(20).asList$OA(pdbfiles)],$I$(1,1).c$$java_util_Collection);
var sl;
for (var i=0; i < this.listeners.size$(); i++) {
if (Clazz.instanceOf(this.listeners.elementAt$I(i), "jalview.structure.StructureListener")) {
...
var atomSpec=Clazz.new_($I$(21,1).c$$S$S$I$I,[pdbfile, chain, pdbResNum, 0]);
var atoms=$I$(22).singletonList$O(atomSpec);
...
this.highlightStructure$jalview_structure_StructureListener$jalview_datamodel_SequenceI$IA(listener, seq, Clazz.array(Integer.TYPE, -1, [seqPos]));
} else {
...
if (results == null ) {
results=$I$(24).buildSearchResults$jalview_datamodel_SequenceI$I$java_util_List(seq, seqPos, this.seqmappings);
...
if (atomNo > 0) {
atoms.add$O(Clazz.new_([sm.pdbfile, sm.pdbchain, sm.getPDBResNum$I(index), atomNo],$I$(21,1).c$$S$S$I$I));
}}
}}
...
return tmp.toArray$OA(Clazz.array($I$(19), [tmp.size$()]));
});

Clazz.newMeth(C$, 'printMappings$S$java_util_List',  function (pdbfile, seqs) {
if (pdbfile == null  || seqs == null   || seqs.isEmpty$() ) {
return "";
}var sb=Clazz.new_($I$(16,1).c$$I,[64]);
for (var sm, $sm = this.mappings.iterator$(); $sm.hasNext$()&&((sm=($sm.next$())),1);) {
if ($I$(25).pathEquals$S$S(sm.pdbfile, pdbfile) && seqs.contains$O(sm.sequence) ) {
...
return $I$(24).mapEditCommand$jalview_commands_EditCommand$Z$jalview_datamodel_AlignmentI$C$java_util_List(command, undo, mapTo, gapChar, this.seqmappings);
} else if (Clazz.instanceOf(command, "jalview.commands.OrderCommand")) {
return $I$(24).mapOrderCommand$jalview_commands_OrderCommand$Z$jalview_datamodel_AlignmentI$java_util_List(command, undo, mapTo, this.seqmappings);
}return null;
});
