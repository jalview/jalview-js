...
$I$(5).warn$S("IMPLEMENTATION ERROR?: Unexpected mapping for unmapped jalview string object content:" + seqsetidobj + " to object " + obj );
}return obj;
} else {
$I$(5).warn$S("Unexpected mapping for Jalview String Object ID " + seqsetidobj + " to another jalview dataset object " + seqsetidobj );
}}}if ($I$(5).isDebugEnabled$()) {
$I$(5).debug$S("Returning null VorbaID binding for jalview object " + jvobj);
}return null;
});
...
$I$(5).debug$S("Registering new object and returning null for getvObj2jv");
return null;
...
$I$(5,"error$S",["Failed to get id for " + (vobj.isRegisterable$() ? "registerable" : "unregisterable") + " object " + vobj ]);
}}if (this.vobj2jv.containsKey$O(vobj.getVorbaId$()) && !(this.vobj2jv.get$O(vobj.getVorbaId$())).equals$O(jvobj) ) {
$I$(5,"debug$S$Throwable",["Warning? Overwriting existing vamsas id binding for " + vobj.getVorbaId$(), Clazz.new_(Clazz.load('Exception').c$$S,[$I$(6).getString$S("exception.overwriting_vamsas_id_binding")])]);
} else if (this.jv2vobj.containsKey$O(jvobj) && !(this.jv2vobj.get$O(jvobj)).equals$O(vobj.getVorbaId$()) ) {
$I$(5,"debug$S$Throwable",["Warning? Overwriting existing jalview object binding for " + jvobj, Clazz.new_(Clazz.load('Exception').c$$S,["Overwriting jalview object binding."])]);
}$I$(5,"debug$S",["Binding " + vobj.getVorbaId$() + " to " + jvobj ]);
...
var dataset=null;
if (jds == null ) {
$I$(5).warn$S("Creating new dataset for an alignment.");
...
throw Clazz.new_($I$(7,1).c$$S,["IMPLEMENTATION ERROR: Cannot map an alignment of sequences from different datasets into a single alignment in the vamsas document."]);
}}}}}
}if (dataset == null ) {
$I$(5,"warn$S",["Creating new vamsas dataset for alignment view " + av.getSequenceSetId$()]);
...
for (var i=0; i < jal.getHeight$(); i++) {
modified=!!(modified|(p$1.syncToAlignmentSequence$jalview_datamodel_SequenceI$uk_ac_vamsas_objects_core_Alignment$java_util_Vector.apply(this, [jal.getSequenceAt$I(i), alignment, docseqs])));
}
if (docseqs.size$() > 0) {
...
$I$(5).warn$S("Group associated sequence annotation is not stored in VAMSAS document.");
continue;
...
$I$(5).info$S("update alignment sequence annotation. not yet implemented.");
} else {
$I$(5).info$S("updated alignment sequence annotation added.");
}}}}
}if ($I$(4).desktop != null ) {
...
$I$(5,"warn$S",["Serious Implementation error - Unbound dataset sequence in alignment: " + jvalsq.getDatasetSequence$()]);
...
alseq.setStart$I(Long.$ival(valseq.getStart$()));
alseq.setEnd$I(Long.$ival(valseq.getEnd$()));
modal=true;
...
modal=true;
}if (modal && $I$(5).isDebugEnabled$() ) {
$I$(5,"debug$S",["Updating apparently edited sequence " + alseq.getName$()]);
}} else {
alseq=Clazz.new_([valseq.getName$(), valseq.getSequence$().replace$C$C(valGapchar, gapChar), Long.$ival(valseq.getStart$()), Long.$ival(valseq.getEnd$())],$I$(21,1).c$$S$S$I$I);
...
$I$(5,"error$S",["Invalid dataset sequence id (null) for alignment sequence " + valseq.getVorbaId$()]);
...
$I$(5).debug$S("Returning client input stream for Jalview from Vamsas Document.");
...
Clazz.newMeth(C$, 'getJarInputStream$',  function () {
$I$(5).debug$S("Returning user input stream for Jalview from Vamsas Document.");
...
$I$(5,"debug$S",["Found " + (views == null  ? " no " : "" + views.length) + " views for '" + av.getSequenceSetId$() + "'" ]);
if (views.length > 1) {
var gathered=false;
var newviewid=null;
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(5).error$S$Throwable("Couldn\'t update jalview client application data. Giving up - local settings probably lost.", e);
} else {
throw e;
}
}
} else {
$I$(5).error$S("Couldn\'t access client application data for vamsas session. This is probably a vamsas client bug.");
}});
...
var jremain=0;
if (jdataset == null ) {
$I$(5).debug$S("Initialising new jalview dataset fields");
newds=true;
dsseqs=Clazz.new_($I$(1,1));
} else {
$I$(5).debug$S("Update jalview dataset from vamsas.");
...
$I$(5).debug$S("New vamsas dataset imported into jalview.");
...
if (dsSeq == null ) {
$I$(5,"warn$S",["Couldn't resolve jalview sequenceI for dataset object reference " + (dataset.getDataSetAnnotations$I(dsa).getSeqRef$I(0)).getVorbaId$().getId$()]);
} else {
...
$I$(5).warn$S("Ignoring dataset annotation with annotationElements. Not yet supported in jalview.");
}}} else {
$I$(5).warn$S("Ignoring multiply referenced dataset sequence annotation for binding to datsaet sequence features.");
}}
...
$I$(5).info$S("UNIMPLEMENTED: not recovering user modifiable sequence alignment annotation");
}}}
}}
if (jal == null ) {
var seqs=Clazz.array($I$(30), [dsseqs.size$()]);
...
$I$(5,"debug$S",["New vamsas alignment imported into jalview " + alignment.getVorbaId$().getId$()]);
...
if (jan != null ) {
$I$(5).debug$S("update from vamsas alignment annotation to existing jalview alignment annotation.");
if (an[j].getModifiable$() == null ) {
$I$(5).info$S("NOT IMPLEMENTED - Recovering user-modifiable annotation - yet...");
}} else {
...
if (av == null ) {
$I$(5,"debug$S",["New alignframe for alignment " + alignment.getVorbaId$()]);
...
$I$(5).warn$S$Throwable("update of labels failed.", e);
} else {
throw e;
}
}
} else {
$I$(5,"warn$S",["Cannot create tree for tree " + t + " in document (" + alignment.getTree$I(t).getVorbaId$() ]);
}}
}}
}}
for (var _ds=0, _nds=root.getDataSetCount$(); _ds < _nds; _ds++) {
...
var pos=Long.$ival(ae[aa].getPosition$()) - 1;
if (pos >= 0 && pos < rangeMap.length ) {
var row=ae[aa].getAfter$() ? 1 : 0;
if (anot[row][pos] != null ) {
$I$(5).debug$S("Ignoring duplicate annotation site at " + pos);
continue;
...
$I$(5).debug$S("ignoring hydrophobicity glyph marker.");
...
$I$(5,"debug$S",["IMPLEMENTATION TODO: Ignoring unknown glyph type " + glyphs[g].getDict$()]);
}}
}var val=0;
if (ae[aa].getValueCount$() > 0) {
AeContent[C$.HASVALS]=true;
if (ae[aa].getValueCount$() > 1) {
$I$(5,"warn$S",["ignoring additional " + (ae[aa].getValueCount$() - 1) + " values in annotation element." ]);
...
$I$(5,"warn$S",["Ignoring out of bound annotation element " + aa + " in " + annotation.getVorbaId$().getId$() ]);
}}
if (mergeable) {
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(5).warn$S("Failed to parse threshold property");
} else {
throw e;
}
}
...
$I$(5).debug$S("Inserting empty annotation row elements for a whole-alignment annotation.");
} else {
if (parsedRangeAnnotation[3] != null ) {
$I$(5,"warn$S",["Ignoring 'After' annotation row in " + annotation.getVorbaId$()]);
...
$I$(5,"warn$S",["Ignoring " + annotation.getLinkCount$() + "links added to AlignmentAnnotation." ]);
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(5).info$S("UNIMPLEMENTED : Couldn\'t parse non-integer group value for setting graphGroup correctly.");
} else {
throw e;
}
}
...
ml=Clazz.new_([localRange, mappedRange, Long.$ival(lu), Long.$ival(mu)],$I$(39,1).c$$IA$IA$I$I);
return ml;
}, p$1);
...
$I$(5).error$S("Can\'t find the correct dataset for the alignment in this view. Creating new one.");
...
$I$(5,"warn$S",["NO Vamsas Binding for local sequence! NOT CREATING MAPPING FOR " + dmps[smp].getDisplayId$Z(true) + " to " + mps[smp].getTo$().getName$() ]);
}}
}}
}} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
