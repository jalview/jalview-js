(function(){var P$=Clazz.newPackage("jalview.io"),I$=[[0,'java.util.ArrayList','java.util.Hashtable','java.util.StringTokenizer','StringBuilder','jalview.util.Comparison','jalview.datamodel.SequenceI','jalview.datamodel.Sequence','jalview.util.Format']],$I$=function(i,n){return((i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
s[i]=Clazz.new_([sqs[i].getName$(), sb.toString(), sqs[i].getStart$(), sqs[i].getEnd$()],$I$(7,1).c$$S$S$I$I);
if (sb.length$() > max) {
max=sb.length$();
}i++;
}
var maxLenpad=Clazz.new_(["%" + ( String.instantialize("" + max)).length$() + "d" ],$I$(8,1).c$$S);
var maxChkpad=Clazz.new_(["%" + ( String.instantialize("1" + max)).length$() + "d" ],$I$(8,1).c$$S);
i=0;
var bigChecksum=0;
...
maxNB=15;
}var nbFormat=Clazz.new_($I$(8,1).c$$S,["%-" + maxNB + "s" ]);
...
out.append$S(Clazz.new_($I$(8,1).c$$S,["%-" + maxid + "s" ]).form$S(name + " "));
for (var k=0; k < 5; k++) {
var start=(i * 50) + (k * 10);
var end=start + 10;
