(function(){var P$=Clazz.newPackage("net.miginfocom.layout"),I$=[[0,'java.util.WeakHashMap','java.util.HashMap']],$I$=function(i,n){return((i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
/*c*/var C$=Clazz.newClass(P$, "LinkHandler");
...
C$.$fields$=[[]
,['O',['LAYOUTS','java.util.WeakHashMap']]]
...
map.put$O$O(key, Clazz.array(Integer.TYPE, -1, [x, y, width, height, x + width, y + height]));
return true;
} else {
var changed=false;
if (x != -2147471302) {
...
var values_temp=Clazz.new_($I$(2,1).c$$I,[4]);
if (temporary) values_temp.put$O$O(key, bounds);
var values=Clazz.new_($I$(2,1).c$$I,[4]);
if (temporary == false ) values.put$O$O(key, bounds);
C$.LAYOUTS.put$O$O(layout, Clazz.array($I$(2), -1, [values, values_temp]));
return true;
}, 1);
