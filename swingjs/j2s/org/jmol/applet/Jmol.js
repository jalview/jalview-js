(function(){var P$=Clazz.newPackage("org.jmol.applet"),p$1={},I$=[[0,'org.jmol.awt.FileDropper','java.util.Hashtable','javax.swing.UIManager','org.jmol.util.Logger','netscape.javascript.JSObject','org.jmol.c.CBK','org.jmol.util.GenericApplet','java.awt.Dimension','java.awt.Color','javajs.util.PT','org.jmol.util.Parser']],$I$=function(i,n,m){return m?$I$(i)[n].apply(null,m):((i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
if (this.getBooleanValue$S$Z("useCommandThread", this.isSigned)) p$1.addValue$java_util_Map$S$S$O.apply(this, [this.vwrOptions, null, "useCommandThread", Boolean.TRUE]);
var options="";
...
if (this.isJNLP) p$1.addValue$java_util_Map$S$S$O.apply(this, [this.vwrOptions, null, "isJNLP", Boolean.TRUE]);
...
try {
$I$(3,"setLookAndFeel$S",[$I$(3).getCrossPlatformLookAndFeelClassName$()]);
} catch (exc) {
System.err.println$S("Error loading L&F: " + exc);
}
if ($I$(4).debugging) {
$I$(4,"debug$S",["checking for jsoWindow mayScript=" + this.mayScript]);
}if (this.mayScript) {
this.mayScript=this.haveDocumentAccess=false;
var jsoWindow=null;
var jsoDocument=null;
try {
jsoWindow=$I$(5).getWindow$java_applet_Applet(this.applet);
if ($I$(4).debugging) {
$I$(4,"debug$S",["jsoWindow=" + jsoWindow]);
}if (jsoWindow == null ) {
$I$(4,"error$S",["jsoWindow returned null ... no JavaScript callbacks :-("]);
} else {
this.mayScript=true;
}jsoDocument=jsoWindow.getMember$S("document");
if (jsoDocument == null ) {
$I$(4,"error$S",["jsoDocument returned null ... no DOM manipulations :-("]);
} else {
...
if ($I$(4).debugging) {
$I$(4).debug$S("jsoWindow:" + jsoWindow + " jsoDocument:" + jsoDocument + " mayScript:" + this.mayScript + " haveDocumentAccess:" + this.haveDocumentAccess );
}}C$.cleanRegistry$();
});
...
for (var item, $item = 0, $$item = $I$(6).values$(); $item<$$item.length&&((item=($$item[$item])),1);$item++) {
if (this.callbacks.get$O(item) != null ) {
haveCallback=true;
break;
}}
if (haveCallback || this.statusForm != null   || this.statusText != null  ) {
if (!this.mayScript) $I$(4).warn$S("MAYSCRIPT missing -- all applet JavaScript calls disabled");
...
$I$(4).info$S("applet text status will be reported to document." + this.statusForm + "." + this.statusText );
}if (this.statusForm != null  && this.statusTextarea != null  ) {
$I$(4).info$S("applet textarea status will be reported to document." + this.statusForm + "." + this.statusTextarea );
}}, p$1);

Clazz.newMeth(C$, 'cleanRegistry$', function () {
var app=null;
var closed=true;
for (var entry, $entry = $I$(7).htRegistry.entrySet$().iterator$(); $entry.hasNext$()&&((entry=($entry.next$())),1);) {
var theApplet=entry.getKey$();
try {
app=(entry.getValue$());
var theWindow=$I$(5).getWindow$java_applet_Applet(app);
closed=(theWindow.getMember$S("closed")).booleanValue$();
if (closed || theWindow.hashCode$() == 0 ) {
}if ($I$(4).debugging) $I$(4,"debug$S",["Preserving registered applet " + theApplet + " window: " + theWindow.hashCode$() ]);
} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
closed=true;
} else {
...
if (closed) {
if ($I$(4).debugging) $I$(4).debug$S("Dereferencing closed window applet " + theApplet);
$I$(7).htRegistry.remove$O(theApplet);
app.destroy$();
}}
}, 1);
...
try {
var jsoWindow=$I$(5).getWindow$java_applet_Applet(this.applet);
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(4,"error$S",["error indicating status at document." + this.statusForm + "." + this.statusText + ":" + e.toString() ]);
} else {
throw e;
}
}
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(4,"error$S",["error indicating status at document." + this.statusForm + "." + this.statusTextarea + ":" + e.toString() ]);
} else {
throw e;
}
}
...
return null;
}var jso=$I$(5).getWindow$java_applet_Applet(this.applet);
...
var mods=$I$(10).split$S$S(callback, ".");
...
try {
this.jsoDocument=$I$(5).getWindow$java_applet_Applet(this.applet).getMember$S("document");
try {
if ((this.jsoDocument.eval$S("!!Jmol._noEval")).booleanValue$()) this.allowJSEval=Boolean.FALSE;
} catch (e) {
if (Clazz.exceptionOf(e,"Exception")){
try {
if ((this.jsoDocument.eval$S("!!_jmol.noEval")).booleanValue$()) this.allowJSEval=Boolean.FALSE;
} catch (e2) {
if (Clazz.exceptionOf(e2,"Exception")){
this.allowJSEval=Boolean.FALSE;
$I$(4,"error$S",["# no Jmol or _jmol object in evaluating " + strEval + ":" + e.toString() ]);
} else {
throw e2;
}
}
...
if (Clazz.exceptionOf(e,"Exception")){
if ($I$(4).debugging) $I$(4).debug$S(" error setting jsoWindow or jsoDocument:" + jsoWindow + ", " + this.jsoDocument );
this.allowJSEval=Boolean.FALSE;
} else {
throw e;
}
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(4,"error$S",["# error evaluating " + strEval + ":" + e.toString() ]);
return "";
} else {
throw e;
}
...
$I$(11).parseStringInfestedFloatArray$S$javajs_util_BS$FA(data, null, fdata);
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(4,"error$S",["Exception " + e.getMessage$() + " with nX, nY: " + nX + " " + nY ]);
} else {
throw e;
}
}
...
if (Clazz.exceptionOf(e,"Exception")){
$I$(4,"error$S",["Exception " + e.getMessage$() + " for " + functionName + " with nX, nY, nZ: " + nX + " " + nY + " " + nZ ]);
} else {
throw e;
}
}
...
this.applet.showStatus$S($I$(10,"rep$S$S$S",[$I$(10).split$S$S(message, "\n")[0], "\'", "\\\'"]));
