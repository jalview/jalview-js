(function(){var P$=Clazz.newPackage("org.jmol.script"),I$=[[0,'java.util.Hashtable','org.jmol.util.Logger','Boolean','javajs.util.Lst','java.util.Arrays','javajs.util.AU']],$I$=function(i,n){return(i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i};
/*c*/var C$=Clazz.newClass(P$, "T");
...
C$.$fields$=[['I',['tok','intValue'],'O',['value','java.lang.Object']]
,['O',['astrType','String[]','tokenSpaceBeforeSquare','org.jmol.script.T','+tokenOn','+tokenOff','+tokenAll','+tokenIf','+tokenAnd','+tokenAndSpec','+tokenOr','+tokenAndFALSE','+tokenOrTRUE','+tokenOpIf','+tokenComma','+tokenDefineString','+tokenPlus','+tokenMinus','+tokenMul3','+tokenTimes','+tokenDivide','+tokenLeftParen','+tokenRightParen','+tokenArraySquare','+tokenArrayOpen','+tokenArrayClose','+tokenLeftBrace','+tokenExpressionBegin','+tokenExpressionEnd','+tokenConnected','+tokenCoordinateBegin','+tokenRightBrace','+tokenCoordinateEnd','+tokenColon','+tokenSetCmd','+tokenSet','+tokenSetArray','+tokenSetProperty','+tokenSetVar','+tokenEquals','+tokenScript','+tokenSwitch','tokenMap','java.util.Map']]]
...
C$.tokenMap.put$O$O(ident, token);
}, 1);
...
if ((token.tok & 4096) != 0 && (s == null  || name.indexOf$S(s) == 0 )  && (isMultiCharacter || (token.value).equals$O(name) ) ) htSet.put$O$O(name, $I$(3).TRUE);
}
...
var name=entry.getKey$();
var token=entry.getValue$();
if (C$.tokAttr$I$I(token.tok, attr) && (notattr == 0 || !C$.tokAttr$I$I(token.tok, notattr) ) ) v.addLast$O(name);
}
var a=v.toArray$OA(Clazz.array(String, [v.size$()]));
$I$(5).sort$OA(a);
return a;
}, 1);
...
if (asCommand ? C$.tokAttr$I$I(tok, 4096) : isSet ? C$.tokAttr$I$I(tok, 536870912) && !C$.tokAttr$I$I(tok, 1610612736)  : true) v.addLast$O(name);
}
...
var t=e.getValue$();
if (C$.tokAttr$I$I(t.tok, attr)) map.put$O$O(e.getKey$(), e.getValue$());
}
}, 1);
...
var t=o;
if (this.tok == t.tok) return (t.intValue == this.intValue && (this.tok == 2 || this.tok == 1073742335  || this.tok == 1073742334  || t.value.equals$O(this.value) ) );
switch (this.tok) {
case 2:
...
C$.tokenMap.put$O$O(lcase, tokenThis);
tokenLast=tokenThis;
}
arrayPairs=null;
...
(function(a,f){return f.apply(null,a)})(["sTokens.length (" + sTokens.length + ") != iTokens.length! (" + iTokens.length + ")" ],$I$(2).error$S);
System.exit$I(1);
}n=sTokens.length;
for (var i=0; i < n; i++) {
sTok=sTokens[i];
