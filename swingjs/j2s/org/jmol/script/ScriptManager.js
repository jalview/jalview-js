...
scriptItem.addLast$O(isQuiet ? Boolean.TRUE : Boolean.FALSE);
...
try {
$I$(1).sleep$J(100);
if (((n++) % 10) == 0) if ($I$(5).debugging) {
$I$(5,"debug$S",["...scriptManager waiting for queue: " + this.scriptQueue.size$() + " thread=" + $I$(1).currentThread$().getName$() ]);
}} catch (e) {
...
if ($I$(5).debugging) $I$(5,"debug$S",[this.scriptQueue.size$() + " scripts; removed: " + strScript ]);
}}
}, p$1);
...
this.queueThreads[pt]=Clazz.new_($I$(6,1).c$$org_jmol_script_ScriptManager$org_jmol_viewer_Viewer$Z$I,[this, this.vwr, startedByCommandWatcher, pt]);
...
$I$(5,"debug$S",["command watcher " + (isStart ? "started" : "stopped") + this.commandWatcherThread ]);
}});
...
var outputBuffer=(statusList == null  || statusList.equals$O("output")  ? Clazz.new_($I$(7,1)) : null);
var oldStatusList=this.vwr.sm.statusList;
this.vwr.getStatusChanged$S(statusList);
if (this.vwr.isSyntaxCheck) $I$(5,"info$S",["--checking script:\n" + this.eval.getScript$() + "\n----\n" ]);
...
if (strErrorMessage == null ) $I$(5).info$S("--script check ok");
 else $I$(5).error$S("--script check error\n" + strErrorMessageUntranslated);
$I$(5,"info$S",["(use \'exit\' to stop checking)"]);
}this.isScriptQueued=true;
...
}$I$(5,"info$S",[this.vwr.isSyntaxCheck ? haltType + " -- stops script checking" : (isInsert ? "!" : "") + haltType + " received" ]);
...
if (type == null ) {
type=$I$(8,"determineSurfaceTypeIs$java_io_InputStream",[this.vwr.getBufferedInputStream$S(fileName)]);
...
return this.vwr.getModelAdapter$().getFileTypeName$O($I$(9).getBR$S(zipDirectory));
...
return $I$(11).getZipDirectoryAsStringAndClose$java_io_BufferedInputStream(t);
}, p$1);

Clazz.newMeth(C$, 'setStateScriptVersion$org_jmol_viewer_Viewer$S', function (vwr, version) {
if (version != null ) {
C$.prevCovalentVersion=$I$(12).bondingVersion;
...
if (modelIndex != this.vwr.ms.mc - 1) return Clazz.new_($I$(13,1));
...
var sbConnect=Clazz.new_($I$(7,1));
