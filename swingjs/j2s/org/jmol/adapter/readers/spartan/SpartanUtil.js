...
if ((token=tokens.nextToken$()).equals$O(")")) v.addLast$O(lastToken);
 else if (token.equals$O("Start-") && tokens.nextToken$().equals$O("Molecule") ) v.addLast$O($I$(7,"split$S$S",[tokens.nextToken$(), "\""])[1]);
...
var br=$I$(4,"getBufferedReader$java_io_BufferedInputStream$S",[$I$(4).isGzipS$java_io_InputStream(bis) ? Clazz.new_([$I$(6).newGZIPInputStream$java_io_InputStream(bis)],$I$(11,1).c$$java_io_InputStream) : bis, null]);
var line;
sb=Clazz.new_($I$(2,1));
if (header != null ) sb.append$S("BEGIN Directory Entry " + path + "\n" );
