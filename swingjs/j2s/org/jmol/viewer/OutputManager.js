...
try {
if (type.equals$O("Gif") && this.vwr.getBoolean$I(603979962) ) params.put$O$O("reducedColors", Boolean.TRUE);
var w=objImage == null  ? -1 : $I$(8).isAI$O(objImage) ? (params.get$O("width")).intValue$() : this.vwr.apiPlatform.getImageWidth$O(objImage);
var h=objImage == null  ? -1 : $I$(8).isAI$O(objImage) ? (params.get$O("height")).intValue$() : this.vwr.apiPlatform.getImageHeight$O(objImage);
...
params.put$O$O("logging", Boolean.valueOf$Z($I$(1).debugging));
...
Clazz.newMeth(C$, 'encodeImage$I$I$O', function (w, h, objImage) {
return (w < 0 ? null : ($I$(8).isAI$O(objImage) ? objImage : this.vwr.apiPlatform.grabPixels$O$I$I$IA(objImage, w, h, (!$I$(4).isJS || $I$(4).isSwingJS  ? Clazz.array(Integer.TYPE, [w * h]) : null))));
}, p$1);
...
if (!this.vwr.haveAccess$org_jmol_viewer_Viewer_ACCESS($I$(9).ALL)) return null;
...
var localName=(isCache || isRemote || $I$(10).isLocal$S(fileName)   ? fileName : null);
try {
...
var is2D=params.get$O("is2D") === Boolean.TRUE ;
var localName=null;
...
params.put$O$O("fileName", fileName);
if ($I$(10).isLocal$S(fileName)) localName=fileName;
...
break;
case 0:
if (Boolean.FALSE === params.get$O("captureEnabled") ) {
...
case 6:
params=cparams;
params.put$O$O("captureEnabled", (captureMode.equals$O("on") ? Boolean.TRUE : Boolean.FALSE));
...
this.vwr.captureParams=null;
params.put$O$O("captureMsg", $I$(12).$$S("Capture") + ": " + (captureMode.equals$O("cancel") ? $I$(12).$$S("canceled") : $I$(12,"o$S$O",[$I$(12).$$S("{0} saved"), fileName])) );
...
path=this.getLogPath$S(this.vwr.logFilePath + value);
 ...
 else $I$(1,"info$S",[$I$(12,"o$S$O",[$I$(12).$$S("Setting log file to {0}"), path])]);
if (value == null  || !this.vwr.haveAccess$org_jmol_viewer_Viewer_ACCESS($I$(9).ALL) ) {
$I$(1,"info$S",[$I$(12).$$S("Cannot set log file path.")]);
value=null;
} else {
...
$I$(1).info$S(data);
return;
}var out=(this.vwr.haveAccess$org_jmol_viewer_Viewer_ACCESS($I$(9).ALL) ? this.openOutputChannel$D$S$Z$Z(this.privateKey, this.vwr.logFileName, true, !doClear) : null);
if (!doClear) {
...
var v=Clazz.new_($I$(13,1));
var fm=this.vwr.fm;
var fileNames=Clazz.new_($I$(13,1));
...
var newFileNames=Clazz.new_($I$(13,1));
for (var iFile=0; iFile < nFiles; iFile++) {
var name=fileNames.get$I(iFile);
var isLocal=!$I$(4).isJS && $I$(10).isLocal$S(name) ;
var newName=name;
...
if (!$I$(8).isAB$O(ret)) return "ERROR: " + ret;
...
var crcValue=Integer.valueOf$I($I$(14).getCrcValue$BA(ret));
...
var fm=this.vwr.fm;
var zos=$I$(14).getZipOutputStream$O(bos);
...
if (data != null ) bytes=($I$(8).isAB$O(data) ? data : (data).getBytes$());
...
$I$(14).addZipEntry$O$S(zos, fnameShort);
var nOut=0;
if (bytes == null ) {
