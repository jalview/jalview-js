...
this.htGroupBonds.put$O$O(group3, Boolean.TRUE);
...
$I$(15).sort$OA$java_util_Comparator(bondInfo, this);
var t;
for (var i=0; i < n; ) {
t=bondInfo[i];
var a1=t[0];
...
$I$(16,"getPlaneThroughPoints$javajs_util_T3$javajs_util_T3$javajs_util_T3$javajs_util_V3$javajs_util_V3$javajs_util_P4",[htAtoms.get$O(name1), htAtoms.get$O(name), htAtoms.get$O(name2), this.vNorm, this.vAB, this.plane]);
var d=$I$(16,"distanceToPlane$javajs_util_P4$javajs_util_T3",[this.plane, htAtoms.get$O(bondInfo[pt][1])]) * factor;
...
bondInfo[i]=null;
continue;
}if (t != null ) $I$(17).info$S(" ligand " + group3 + ": " + bondInfo[i][0] + " - " + bondInfo[i][1] + " order " + bondInfo[i][2] );
}
return bondInfo;
}, p$1);
...
this.bsAddedMask=$I$(18).copy$javajs_util_BS(this.bsAddedHydrogens);
...
$I$(17,"info$S",["Error adding H atoms to " + gName + g.getResno$() + ": " + pts[i].length + " atoms should not be added to " + aName ]);
continue;
...
d=$I$(16,"distanceToPlane$javajs_util_P4$javajs_util_T3",[$I$(16).getPlaneThroughPoints$javajs_util_T3$javajs_util_T3$javajs_util_T3$javajs_util_V3$javajs_util_V3$javajs_util_P4(atom1, atom, atom2, this.vNorm, this.vAB, this.plane), pts[i][0]]) * factor;
break;
}
if (pt < 0) {
$I$(17,"info$S",["Error adding H atoms to " + gName + g.getResno$() + ": expected to only need 1 H but needed 2" ]);
hName1=hName2="H";
} else if (d < 0 ) {
...
this.ms.fixFormalCharges$javajs_util_BS($I$(18).newBitSet2$I$I(this.ml.baseAtomIndex, this.ml.ms.ac));
}, p$1);
...
this.bsAddedMask=$I$(18).deleteBits$javajs_util_BS$javajs_util_BS(this.bsAddedMask, bsDeletedAtoms);
...
htKeysUsed.put$O$O(skey, Boolean.TRUE);
bonds[i].setOrder$I($I$(19).parseInt$S(type));
}
for (var key, $key = this.htBondMap.keySet$().iterator$(); $key.hasNext$()&&((key=($key.next$())),1);) {
if (htKeysUsed.get$O(key) != null ) continue;
if (key.indexOf$S(":") < 0) {
htKeysUsed.put$O$O(key, Boolean.TRUE);
continue;
}var value=this.htBondMap.get$O(key);
$I$(17,"info$S",["bond " + key + " was not used; order=" + value ]);
...
bonds[i].setOrder$I($I$(19).parseInt$S(value));
$I$(17).info$S("assigning order " + bonds[i].order + " to bond " + bonds[i] );
}
}, p$1);
...
var monomers=Clazz.array($I$(20), [count]);
for (var j=0; j < count; ++j) monomers[j]=groups[firstGroupIndex + j];

if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.AminoMonomer")) return Clazz.new_($I$(21,1).c$$org_jmol_modelsetbio_MonomerA$I,[monomers, pt0]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.AlphaMonomer")) return Clazz.new_($I$(22,1).c$$org_jmol_modelsetbio_MonomerA$I,[monomers, pt0]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.NucleicMonomer")) return Clazz.new_($I$(23,1).c$$org_jmol_modelsetbio_MonomerA,[monomers]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.PhosphorusMonomer")) return Clazz.new_($I$(24,1).c$$org_jmol_modelsetbio_MonomerA,[monomers]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.CarbohydrateMonomer")) return Clazz.new_($I$(25,1).c$$org_jmol_modelsetbio_MonomerA,[monomers]);
$I$(17,"error$S",["Polymer.allocatePolymer() ... no matching polymer for monomor " + previous]);
...
if (C$.group3NameCount == $I$(3).group3Names.length) $I$(3).group3Names=$I$(26,"doubleLengthS$SA",[$I$(3).group3Names]);
