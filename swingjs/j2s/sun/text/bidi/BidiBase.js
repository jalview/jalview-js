(function(){var P$=Clazz.newPackage("sun.text.bidi"),p$1={},I$=[[0,['sun.text.bidi.BidiBase','.Point'],'AssertionError',['java.text.AttributedCharacterIterator','.Attribute'],['sun.text.bidi.BidiBase','.ImpTabPair'],'sun.text.bidi.BidiRun',['sun.text.bidi.BidiBase','.InsertPoints'],'sun.text.normalizer.UBiDiProps','java.lang.reflect.Array','OutOfMemoryError','sun.text.normalizer.UTF16',['sun.text.bidi.BidiBase','.LevState'],['sun.text.bidi.BidiBase','.TextAttributeConstants'],['sun.text.bidi.BidiBase','.NumericShapings'],'sun.text.bidi.BidiLine','java.util.Arrays','sun.text.normalizer.UCharacter','StringBuilder']],$I$=function(i,n,m){return m?$I$(i)[n].apply(null,m):((i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
$I$(15).sort$JA(keys);
for (i=0; i < count; i++) {
this.logicalToVisualRunsMap[i]=((keys[i] & -1)|0);
}
keys=null;
...
try {
C$.shapeMethod.invoke$O$OA(shaper, [text, new Integer(start), new Integer(count)]);
} catch (e$$) {
...
C$.shapeMethod=C$.getMethod$Class$S$ClassA(C$.clazz, "shape", [Clazz.array(Character.TYPE, -1), Integer.TYPE, Integer.TYPE]);
};
