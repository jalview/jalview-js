(function(){var P$=Clazz.newPackage("sun.util.locale.provider"),I$=[[0,'sun.util.locale.provider.JRELocaleProviderAdapter','sun.util.locale.provider.SPILocaleProviderAdapter','java.util.concurrent.ConcurrentHashMap','java.security.AccessController','sun.security.action.GetPropertyAction','java.util.ArrayList',['sun.util.locale.provider.LocaleProviderAdapter','.Type'],'java.util.Locale','sun.util.cldr.CLDRLocaleProviderAdapter','sun.util.locale.provider.HostLocaleProviderAdapter','sun.util.locale.provider.LocaleServiceProviderPool','sun.util.locale.provider.FallbackLocaleProviderAdapter','java.util.Collections','InternalError',['java.util.ResourceBundle','.Control'],'sun.util.locale.provider.JRELocaleConstants']],$I$=function(i,n,m){return m?$I$(i)[n].apply(null,m):((i=(I$[i]||(I$[i]=Clazz.load(I$[0][i])))),!n&&i.$load$&&Clazz.load(i,2),i)};
...
default:
throw Clazz.new_($I$(14,1).c$$S,["unknown locale data adapter type"]);
}
}, 1);
...
return adapter;
}var lookupLocales=$I$(15,"getControl$java_util_List",[$I$(15).FORMAT_DEFAULT]).getCandidateLocales$S$java_util_Locale("", locale);
...
Clazz.assert(C$, this, function(){return type === $I$(7).JRE  || type === $I$(7).CLDR   || type === $I$(7).FALLBACK  });
if ($I$(8).ROOT.equals$O(locale)) {
return true;
}if (type === $I$(7).FALLBACK ) {
return false;
...
var locs=Clazz.array($I$(8), [tags.size$() + 1]);
var index=0;
locs[index++]=$I$(8).ROOT;
for (var tag, $tag = tags.iterator$(); $tag.hasNext$()&&((tag=($tag.next$())),1);) {
switch (tag) {
case "ja-JP-JP":
locs[index++]=$I$(16).JA_JP_JP;
break;
case "th-TH-TH":
locs[index++]=$I$(16).TH_TH_TH;
break;
default:
locs[index++]=$I$(8).forLanguageTag$S(tag);
break;
}
}
return locs;
...
{
var order=$I$(4,"doPrivileged$java_security_PrivilegedAction",[Clazz.new_($I$(5,1).c$$S,["java.locale.providers"])]);
var typeList=Clazz.new_($I$(6,1));
if (order != null  && order.length$() != 0 ) {
var types=order.split$S(",");
for (var type, $type = 0, $$type = types; $type<$$type.length&&((type=($$type[$type])),1);$type++) {
try {
var aType=$I$(7,"valueOf$S",[type.trim$().toUpperCase$java_util_Locale($I$(8).ROOT)]);
switch (aType) {
case $I$(7).CLDR:
if (C$.cldrLocaleProviderAdapter == null ) {
C$.cldrLocaleProviderAdapter=Clazz.new_($I$(9,1));
}break;
case $I$(7).HOST:
if (C$.hostLocaleProviderAdapter == null ) {
C$.hostLocaleProviderAdapter=Clazz.new_($I$(10,1));
}break;
}
...
$I$(11,"config$Class$S",[Clazz.getClass(C$), e.toString()]);
} else {
throw e;
}
}
...
if (!typeList.contains$O($I$(7).JRE)) {
C$.fallbackLocaleProviderAdapter=Clazz.new_($I$(12,1));
typeList.add$O($I$(7).FALLBACK);
C$.defaultLocaleProviderAdapter=$I$(7).FALLBACK;
} else {
C$.defaultLocaleProviderAdapter=$I$(7).JRE;
}} else {
typeList.add$O($I$(7).JRE);
typeList.add$O($I$(7).SPI);
C$.defaultLocaleProviderAdapter=$I$(7).JRE;
}C$.adapterPreference=$I$(13).unmodifiableList$java_util_List(typeList);
};
};
;
...
;Clazz.setTVer('3.2.9-v1');//Created 2020-04-08 07:28:51 Java2ScriptVisitor version 3.2.9-v1 net.sf.j2s.core.jar version 3.2.9-v1
