...
preferred+=(iter.getPreferredSpan$F(0)|0);
minimum=(minimum+(iter.getMinimumSpan$F(0))|0);
maximum=(maximum+(iter.getMaximumSpan$F(0))|0);
...
if (r == null ) {
r=Clazz.new_($I$(34,1));
}r.minimum=(minimum > 2147483647) ? 2147483647 : (minimum|0);
r.preferred=(preferred > 2147483647) ? 2147483647 : (preferred|0);
r.maximum=(maximum > 2147483647) ? 2147483647 : (maximum|0);
return r;
}, 1);
...
currentPreferred=(iter.getPreferredSpan$F(targetSpan)|0);
iter.setSpan$I((currentPreferred|0));
preferred+=currentPreferred;
gain[iter.getAdjustmentWeight$()]+=(iter.getMaximumSpan$F(targetSpan)|0) - currentPreferred;
loss[iter.getAdjustmentWeight$()]+=currentPreferred - (iter.getMinimumSpan$F(targetSpan)|0);
lastMargin=(iter.getTrailingCollapseSpan$()|0);
}
totalSpacing+=lastMargin;
totalSpacing=(totalSpacing+(2 * iter.getBorderWidth$())|0);
...
var desiredAdjustment=allocated - preferred;
var adjustmentsArray=(desiredAdjustment > 0) ? gain : loss;
...
var maximumAdjustment=adjustmentsArray[adjustmentLevel] - ((adjustmentLevel > 0) ? adjustmentsArray[adjustmentLevel - 1] : 0);
...
iter.setSpan$I((((allocated > preferred) ? Math.floor(iter.getMaximumSpan$F(targetSpan)) : Math.ceil(iter.getMinimumSpan$F(targetSpan)))|0));
} else if (iter.getAdjustmentWeight$() == adjustmentLevel) {
var availableSpan=(allocated > preferred) ? (iter.getMaximumSpan$F(targetSpan)|0) - iter.getSpan$() : iter.getSpan$() - (iter.getMinimumSpan$F(targetSpan)|0);
var adj=(Math.floor(adjustmentFactor * availableSpan)|0);
iter.setSpan$I(iter.getSpan$() + ((allocated > preferred) ? adj : -adj));
}totalOffset=(Math.min(iter.getOffset$() + iter.getSpan$(), 2147483647)|0);
}
...
ss=p$1.getStyleSheet$javax_swing_text_html_StyleSheet.apply(this.this$0, [ss]);
if (this.index) {
return Math.round(this.this$0.getPointSize$I$javax_swing_text_html_StyleSheet.apply(this.this$0, [(this.value|0), ss]));
...
fs.svalue=value;
try {
var baseFontSize=this.this$0.getBaseFontSize$.apply(this.this$0, []);
if (value.charAt$I(0) == "+") {
...
}return $I$(2,"getValue$F$S$Boolean",[this.span, this.units, new Boolean(isW3CLengthUnits)]);
});
